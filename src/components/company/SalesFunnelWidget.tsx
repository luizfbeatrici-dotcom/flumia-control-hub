...
export function SalesFunnelWidget({ empresaId, isMinimized, onToggleMinimize }: SalesFunnelWidgetProps) {
  ...
  useEffect(() => {
    if (user?.id) {
      const savedPreference = localStorage.getItem(`kanban-hide-empty-lanes-${user.id}`);
  ...
  const handleToggleEmptyLanes = (checked: boolean) => {
    ...
    if (user?.id) {
      localStorage.setItem(`kanban-hide-empty-lanes-${user.id}`, String(checked));
    }
  };
